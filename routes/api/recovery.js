...
const {
  ...
  getGasEstimates,
  getGaslessEstimates,
} = require("../../utils/contracts/recovery");
...
router.post("/native/:chainId", async (req, res) => {
  try {
    ...
    if (txHash === forwardRequest.newTxHash) {
      return res.json({
        success: false,
        error: "TxHash should not be the same as newTxHash",
      });
    }
    ...
    const isSignatureValid = await verifyExecuteRecoveryPayload(
      forwardRequest,
      forwarder,
      currentChain
    );
